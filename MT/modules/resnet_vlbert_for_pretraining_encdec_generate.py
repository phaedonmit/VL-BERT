...
from common.visual_linguistic_bert import VisualLinguisticBertEncoder
from common.visual_linguistic_bert_decoder import VisualLinguisticBertForPretrainingDecoder
from common.utils.misc import soft_cross_entropy
# from external.pytorch_pretrained_bert import BertModel, BertConfig
# from transformers import BertModel, BertConfig

BERT_WEIGHTS_NAME = 'pytorch_model.bin'


class ResNetVLBERTForPretrainingEncDecGenerate(Module):
    ...
    def train(self, mode=True):
        super(ResNetVLBERTForPretrainingEncDecGenerate, self).train(mode)
    ...
    def forward(self,
        ...
        text_visual_embeddings_de = self._collect_obj_reps(text_tags_de, obj_reps['obj_reps'])

        object_linguistic_embeddings_de = self.object_linguistic_embeddings(
            boxes.new_zeros((boxes.shape[0], boxes.shape[1])).long()
        )
        if self.config.NETWORK.WITH_MVRC_LOSS:
            object_linguistic_embeddings_de[mvrc_ops == 1] = self.object_mask_word_embedding_de.weight[0]
        object_vl_embeddings_de = torch.cat((obj_reps['obj_reps'], object_linguistic_embeddings_de), -1)




        # FM added - for generation ###########################

                ###########################################
        # Visual Linguistic BERT
        # #loop here for test mode:
        generated = []
        stop = [False]*text_de.shape[0]
        curr_len = 0
        max_len = 48

        # Visual Linguistic BERT - Encoder
        relationship_logits_en, mlm_logits_en, mvrc_logits_en, encoder_hidden_states = self.vlbert(text_input_ids,
                                                                text_token_type_ids,
        ...
        while not all(stop) and curr_len<=max_len:
            ...
            relationship_logits, mlm_logits, mvrc_logits = self.decoder(text_input_ids_de,
                                                                    text_token_type_ids_de,
                                                                    text_visual_embeddings_de,
                                                                    text_mask_de,
                                                                    object_vl_embeddings_de,
                                                                    box_mask, encoder_hidden_states)

            answers = torch.topk(mlm_logits[mlm_labels_de==103], k=1,  dim=1)
            ...
            position_tensor = torch.arange(mlm_labels_de.shape[1])
            position_tensor = position_tensor.repeat(mlm_labels_de.shape[0]).view(mlm_labels_de.shape[0],-1)
            indeces = position_tensor[mlm_labels_de==103]

            # 1. Update mlm_labels:
            mlm_labels_new = mlm_labels_de.new_zeros(mlm_labels_de.shape[0], mlm_labels_de.shape[1]+1)
            mlm_labels_new = mlm_labels_new - 1
            mlm_labels_new[torch.arange(mlm_labels_de.shape[0]), indeces+1] = 103
            mlm_labels_de = mlm_labels_new

            # 2. Update text_input_ids:
            text_input_ids_new = text_input_ids_de.new_zeros(text_input_ids_de.shape[0], text_input_ids_de.shape[1]+1)
            text_input_ids_new[:, :-1] = text_input_ids_de
            text_input_ids_new[torch.arange(text_input_ids_de.shape[0]), indeces] = answers[1][:,0]
            text_input_ids_new[torch.arange(text_input_ids_de.shape[0]), indeces+1] = (self.tokenizer.convert_tokens_to_ids(['[MASK]'])[0])
            text_input_ids_new[torch.arange(text_input_ids_de.shape[0]), indeces+2] = (self.tokenizer.convert_tokens_to_ids(['[PAD]'])[0])
            text_input_ids_new[torch.arange(text_input_ids_de.shape[0]), indeces+3] = (self.tokenizer.convert_tokens_to_ids(['[SEP]'])[0])
            text_input_ids_de = text_input_ids_new

            # 3. Update text_token_type_ids:
            text_token_type_ids_de = text_token_type_ids_de.new_zeros(text_token_type_ids_de.shape[0], text_token_type_ids_de.shape[1]+1)

            # 4. Update text_input_ids:
            text_visual_embeddings_new = text_visual_embeddings_de.new_zeros(text_visual_embeddings_de.shape[0], text_visual_embeddings_de.shape[1]+1, text_visual_embeddings_de.shape[2])
            text_visual_embeddings_new = text_visual_embeddings_new.transpose(0,1)
            text_visual_embeddings_new[:] = text_visual_embeddings_de[:,0,:]
            text_visual_embeddings_de = text_visual_embeddings_new.transpose(0,1)
        ...
        if self.config.NETWORK.WITH_MLM_LOSS:
            mlm_logits_padded = mlm_logits.new_zeros((*mlm_labels_de.shape, mlm_logits.shape[-1])).fill_(-10000.0)
            ...
            if self.config.NETWORK.MLM_LOSS_NORM_IN_BATCH_FIRST:
                ...
                num_mlm = (mlm_labels_de != -1).sum(1, keepdim=True).to(dtype=mlm_loss.dtype)
        ...
        outputs.update({
            ...
            'mlm_label': mlm_labels_de if self.config.NETWORK.WITH_MLM_LOSS else None,
