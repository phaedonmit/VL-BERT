...
class Multi30kDatasetDecoder(Dataset):
    def __init__(self, ann_file, image_set, root_path, data_path, seq_len=64,
        ...
        :param kwargs:
        """
        super(Multi30kDatasetDecoder, self).__init__()
    ...
    @property
    def data_names(self):
        return ['image', 'boxes', 'im_info', 'text_en', 'text_de',
                'relationship_label', 'mlm_labels_en', 'mlm_labels_de', 'mvrc_ops', 'mvrc_labels']
    ...
    def __getitem__(self, index):
        ...
        text_tokens_en = ['[CLS]'] + caption_tokens_en + ['[SEP]'] 
        mlm_labels_en = [-1] + mlm_labels_en + [-1]
        text_tokens_de = ['[CLS]'] + caption_tokens_de + ['[SEP]'] 
        mlm_labels_de = [-1] + mlm_labels_de + [-1]
        ...
        text_en = self.tokenizer.convert_tokens_to_ids(text_tokens_en)
        text_de = self.tokenizer.convert_tokens_to_ids(text_tokens_de)

        if self.with_precomputed_visual_feat:
            boxes = torch.cat((boxes, boxes_features), dim=1)

        # truncate seq to max len
        if len(text_en) + len(boxes) > self.seq_len:
            text_len_keep = len(text_en)
            ...
            text_en = text_en[:(text_len_keep - 1)] + [text_en[-1]]
            mlm_labels_en = mlm_labels_en[:(text_len_keep - 1)] + [mlm_labels_en[-1]]
            mvrc_ops = mvrc_ops[:box_len_keep]
            mvrc_labels = mvrc_labels[:box_len_keep]

        return image, boxes, im_info, text_en, text_de, relationship_label, mlm_labels_en, mlm_labels_de, mvrc_ops, mvrc_labels
