...
class Translation_Multi30kDataset(Dataset):
    def __init__(self, ann_file, image_set, root_path, data_path, seq_len=64,
        ...
        :param kwargs:
        """
        super(Translation_Multi30kDataset, self).__init__()
        ...
        # FM edit: create dataset for test mode 
            ...
            for x, idb_x in enumerate(self.simple_database):
                for y, idb_y in enumerate(self.simple_database):                    
                    ...
                    self.database[db_index]['image'] = self.simple_database[x]['image']
                    self.database[db_index]['frcnn'] = self.simple_database[x]['frcnn']
    ...
    def data_names(self):
        return ['image', 'boxes', 'im_info', 'text',
                'relationship_label', 'mlm_labels', 'mvrc_ops', 'mvrc_labels', 'caption_en_index', 'caption_de_index']
    ...
    def __getitem__(self, index):
        ...
        return image, boxes, im_info, text, relationship_label, mlm_labels, mvrc_ops, mvrc_labels, caption_en_index, caption_de_index
