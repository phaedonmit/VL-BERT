...
# FM: Added to work in Docker container:
    ...
    if split_name == 'conceptual_captions_train':
      ...
      with open(os.path.join(data_root, 'utils/val.json')) as f:
        for cnt, line in enumerate(f):  
          ...
          image_id = int(filepath.split('/')[-1][:-4])
          split.append((filepath,image_id))   
    # FM: added image loader for multi30k/flickr30k dataset
    elif split_name == 'flickr30k_train':
      with open(os.path.join(data_root, 'flickr30k/utils/train.txt')) as f:
        for cnt, line in enumerate(f):  
          filepath = os.path.join('flickr30k/flickr30k-images.zip@/', line.strip())
          image_id = int(line.split('.')[0])
          split.append((filepath,image_id))   
    elif split_name == 'flickr30k_val':
      with open(os.path.join(data_root, 'flickr30k/utils/val.txt')) as f:
        for cnt, line in enumerate(f):  
          filepath = os.path.join('flickr30k/flickr30k-images.zip@/', line.strip())
          image_id = int(line.split('.')[0])
          split.append((filepath,image_id))   
